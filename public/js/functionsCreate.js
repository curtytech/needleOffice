...
addServico = function () {
    ...
    cols += '<div class="col-md-2 ">  <label> Valor do Serviço: ' + counterServico + ' </label> ' +
        ' <input class="form-control moeda valServico " type="text" name="valServico[]" id="valServico" /></div> ';
...
addMaterial = function () {
    ...
    $(".valMaterial, .qtdMaterial").on("blur keyup", function () {
        ...
        const valMaterial = parseFloat(tr.find('.valMaterial').val());
        const qtdMaterial = parseFloat(tr.find('.qtdMaterial').val());


        if (!isNaN(valMaterial) && !isNaN(qtdMaterial)) {
            tr.find('.valTotalMaterial').val((valMaterial * qtdMaterial));
